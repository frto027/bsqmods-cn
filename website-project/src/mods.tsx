      ...
      <div className='p-5 bg-body-tertiary rounded-3 title-bg'>
        <div className='container-fluid py-5'>
          <h1 className='display-5 fw-bold' >
            <span>bs</span> <span>qmods</span>
          </h1>
          <div className='col-md-8 fs-4'>BeatSaber Quest mod sources <br/>(en ver preview only, part of chinese local project)</div>
        </div>
      </div>
...
function Body() {
  ...
  return <div className="my-container">
    <div className="alert alert-light mt-3" style={{display:"none"}} role="alert">
    ...
    <div className='alert alert-light fade show pb-3 pb-sm-2'>
      <b className=''><i className="m-1 bi bi-toggles"></i>Settings</b>
      ...
      <div className='ms-2 row'>
          ...
          <div className="form-check form-switch col col-12 col-sm-6 col-lg-3">
            ...
            <label className="form-check-label" htmlFor="showOldGameSwitch"><i className="me-1 bi bi-clock-history"></i>Old versions</label>
          </div>
          ...
          <div className="form-check form-switch col col-12 col-sm-6 col-lg-3">
            ...
            <label className="form-check-label" htmlFor="showNonCoreGameSwitch"><i className="me-1 bi bi-patch-exclamation"></i>Non avaliable versions</label>
          </div>
          ...
          <div className="form-check form-switch col col-12 col-sm-6 col-lg-3">
            ...
            <label className="form-check-label" htmlFor="enableMasonary"><i className="me-1 bi bi-columns-gap"></i>Masonary render<span className='ms-2 badge text-bg-warning p-1' style={{fontSize:"10px", verticalAlign:"text-bottom"}}>High CPU usage</span></label>
          </div>
          ...
          <div className="form-check form-switch col col-12 col-sm-6 col-lg-3">
            ...
            <label className="form-check-label" style={{ userSelect: "none" }} htmlFor="dayNightColorToggle"><i className="me-1 bi bi-moon"></i>Dark mode</label>
          </div>
      ...
      <div className='px-2 small d-flex flex-column flex-sm-row justify-content-between'>
        <span  className='text-secondary' style={{verticalAlign:"sub", textWrap:"nowrap"}}>{buildDate} (last sync)</span>
...
Version Select</b>
      <div className="">
          ...
          <small className='text-secondary'><p className='mb-1' style={{ textIndent: "0em" }}>Always install/downgrade to <b>latest avaliable version</b>, and upgrade your mods to enjoy <b>mod update</b> and <b>bug fixes</b>.Take is easy. Your song and datas will not be deleted during game update/uninstall.</p></small>

          <small className='text-secondary'><p className='mt-1 mb-0' style={{ textIndent: "0em" }}>Current ver: {show_version}</p></small>
      </div>
    </div>
    ...
    <div style={{
      ...
      <b>Version not avaliable<i className="m-1 bi bi-exclamation-octagon-fill"></i></b><br />
      Version {show_version} can't be used in MBF or QuestPatcher, because core mods is not ready.
    </div>
        ...
        <div className='col col-12 col-md-6 soft-card'>
          <div className='card rounded-4 h-100 w-100'>
            <div className='card-body'>
              ...
              <hr/>
              <p className="card-text mt-2 mb-1" style={{ textIndent: "0em" }}>这是用于Quest一体机的节奏光剑模组安装工具。由中文社区开发者维护的补丁工具。可以下载安装后使用。</p>
              <p className="card-text mt-2 mb-1" style={{ textIndent: "0em" }}>可选择切换此中文源。</p>
              <div>
        ...
        <div className='col col-12 col-md-6 soft-card'>
          <div className='card rounded-4 h-100 w-100'>
            <div className='card-body'>
              ...
              <hr/>
              <p className="card-text mt-2 mb-1" style={{ textIndent: "0em" }}>这是用于Quest一体机的节奏光剑模组安装工具。此软件（简称MBF）是英文模组社区BSMG主流推荐的模组工具。</p>
              <p className="card-text mt-2 mb-1" style={{ textIndent: "0em" }}>此为中文分支，使用此中文源。</p>
              <div>
    ...
    {/* <hr/> */}
    <div className='text-body-secondary pt-1 pb-0 small' style={{display:"none"}}>
    ...
    <div className='text-body-secondary small'>Page layout is bootstrap.masonic。</div> */}
...
function ModLabels({data, gamever}:{data:ModItem, gamever:string}){
  ...
  if (data._isAddedByCNSource) {
    cn_source = <span className="badge text-bg-info">CN source</span>
  }
  let core_mod = null
  if (isCoreMod(gamever, data.id)) {
    core_mod = <span className="badge text-bg-warning">Core</span>
  }
  let is_library = null
  if (data.isLibrary) {
    is_library = <>&nbsp;<span className="badge text-bg-secondary">Library</span></>
  }
  ...
  const references = []
  {
    ...
    if (data.source && data.source.startsWith("https://")) {
      references.push(<a className='btn btn-link btn-tiny pe-2' key="source" href={data.source} target='_blank'>{icon(data.source)}{
        data.source == data.website ? "Source/Website" : "Source"
      }</a>)
    }
    if (data.website && data.website != data.source && data.website.startsWith("https://")) {
      references.push(<a className='btn btn-link btn-tiny me-2' key="website" href={data.website} target='_blank'>{icon(data.website)}Website</a>)
    }
    if (data.download && data.download.startsWith("https://")) {
      references.push(<a className='btn btn-link btn-tiny m2-1' key="download" href={data.download} target='_blank'><i className="me-1 bi bi-download"></i>Download</a>)
    }
  }
  return references
}
